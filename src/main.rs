...
use burn::{
    backend::{wgpu::AutoGraphicsApi, Autodiff, Wgpu, Fusion},
    module::Module,
    nn::loss::MSELoss,
...
type SelectedBackend = Autodiff<Fusion<Wgpu<AutoGraphicsApi, f32, i32>>>;

fn main() {
    let device: burn::backend::wgpu::WgpuDevice = burn::backend::wgpu::WgpuDevice::default();
    let mut model: YOLOv2Model<SelectedBackend> =
        YOLOv2ModelConfig::new([256, 256], 1, 16, 1).init(&device);
    ...
    for b in batch_sizes_to_test {
        ...
        let batch_input: Tensor<SelectedBackend, 4> = Tensor::random(
            Shape::new([b, 1, 256, 256]),
            burn::tensor::Distribution::Default,
            &device,
        );
        let batch_output: Tensor<SelectedBackend, 2> = Tensor::random(
            Shape::new([b, 16usize.pow(2) * 5]),
            burn::tensor::Distribution::Default,
            &device,
        );
        ...
        for i in 0..=runs_per_batch {
            ...
            if i > 0 {
                ...
                println!(
                    "Epoch {} took {:.3}ms and reached loss {:.3}",
                    i,
        ...
        println!(
            "Average time per batch at batch_size={b}: {:.3}ms",
